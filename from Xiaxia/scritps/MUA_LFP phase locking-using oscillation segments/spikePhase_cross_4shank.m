function R= spikePhase_cross_4shank(experiment,bands,cross,threshold,path)
...
%%
    [SharpStart, SharpEnd] = detection_discont_events_XiaXia(signalRaw, fs,2); % detect oscillations
    num_Oscs=length(SharpStart);
...
%% obtain MUA, chan_num{1}
    ...
    for num_band=1:num_bands
        ...
        for num_Osc=1:num_Oscs
            S=min(find(peakLoc>=SharpStart(1,num_Osc)));
