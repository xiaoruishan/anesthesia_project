...
for n = 1 : length(animal)
    ...
    for period = repetitions
        ...
        if period == 1
            ...
            SampleEntPFC(n, :) = median(InfoTheory.SampEnt1);
            SampleEntHP(n, :) = median(InfoTheory.SampEnt2);
        end
    end
end
...
oscillations = squeeze(mean(reshape(MinuteOsc, 15, size(FR, 1), size(MinuteOsc, 2))));
...
oscillations1 = squeeze(mean(reshape(MinuteOsc, 15, size(FR1, 1), size(MinuteOsc, 2))));
...
summary_one_way = horzcat(anim(:), time(:), FR(:), normFR(:), FR1(:), normFR1(:), oscillations(:), oscillations1(:), slope(:), slope1(:));
...
summary_two_way = horzcat(anim(:), frequency(:), condition(:), powerPFC, powerOscPFC, powerHP, ...
    powerOscHP, Coherence, ppcPFC, ppcHP, entropyPFC, entropyHP);
