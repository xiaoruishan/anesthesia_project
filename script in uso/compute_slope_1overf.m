...
xvalues = repmat(linspace(8, 53, 4)', 1, size(quartodora, 2));

figure; boundedline(linspace(1,60,60), nanmedian(slope,2), 1.25 * nanstd(slope,0,2)./sqrt(size(slope,2))); hold on
scatter(xvalues(:), quartodora(:), 'o', 'filled', 'MarkerFaceColor', 'k')
plot(xvalues, quartodora, 'Color', 'k', 'linewidth', 0.5)
ylim([-3 0]); plot([15 15],get(gca,'ylim'),'r','linewidth',3)
