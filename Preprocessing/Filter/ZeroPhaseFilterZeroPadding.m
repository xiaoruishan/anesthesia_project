...
function recording_filtered=ZeroPhaseFilterZeroPadding(recording_unfiltered,samplingrate,cutFrequencies)
...
% for this function, ask Kay or Samuel for further questions...
    if length(cutFrequencies) ~= 2;
        ...
        recording_unfilteredZeroPadded = padarray(recording_unfiltered,[0,nZeros]);
        ...
        high_cut    = cutFrequencies(2);
            if low_cut == 0;
                ...
                [sos,g]             = zp2sos(b,a,k);
                recording_filtered  = (filtfilt(sos,g,recording_unfilteredZeroPadded));    
            elseif high_cut == 0;
                n                   = 3; 
        ...
        recording_filtered = recording_filtered(nZeros+1:end-nZeros);
    end
end