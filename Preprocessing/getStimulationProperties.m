function getStimulationProperties(experiments,save_data,repeatCalc,expType)
Path = get_path;
for n_animal = 1:length(experiments);
    experiment=experiments(n_animal);
    if ~isempty(experiment.animal_ID)
        if  repeatCalc == 0 && exist(strcat(Path.output, 'results', filesep, 'StimulationProperties', filesep, experiment.name, filesep, 'StimulationProperties_raw','.mat'));
            ...
            [StimTimestamps] = getStimulationTimeStamps(experiment,save_data);
            [~, signalD, ~,~] = nlx_load_Opto(experiment,'Stim1D',[],10,0);
            signalD = digital2binary(signalD);
            [time, signalA, samplingrate,~] = nlx_load_Opto(experiment,'Stim1A',[],10,0);
            ...
            for j           = 1:length(stim_periode_Start);
                ...
                if j ==1;
                    singlePulseStartTime = (stimStart(1:max(find(stimStart < periodeEnd))))./samplingrate*10^6+time(1);
                else
                    singlePulseStartTime = (stimStart(max(find(stimStart < periodeStart))+1:max(find(stimStart < periodeEnd))))./samplingrate*10^6+time(1);
                end
                ...
                if n_pulses > 1;
                    ...
                    if max_mean_ratio <= 0.55; % this number can be changed to fit data best. Ideally 0.50 but higher due to noise
                        ...
                        if round(maxfirsthalf) == round(maxlasthalf)
            ...
            if ~isempty(StimulationProperties_raw)
                ...
                elseif save_data == 1
                    ...
                    save(strcat(Path.output, 'results', filesep, 'StimulationProperties', filesep, experiment.name, filesep, 'StimulationProperties_raw','.mat'),'StimulationProperties_raw');
                end
            else
                disp('StimProperties was not calculated')
            end
%             if strcmp(expType{1},'J2O')
%                 
%                 clearvars StimulationProperties_raw stimPeriodeStart stimPeriodeEnd
%                 
%                 [StimTimestamps] = getStimulationTimeStamps2(experiment,save_data);
%                 [~, signalD, ~,~] = nlx_load_Opto(experiment,'Stim2D',[],10,0);
%                 signalD = digital2binary(signalD);
%                 [time, signalA, samplingrate,~] = nlx_load_Opto(experiment,'Stim2A',[],10,0);
%                 
%                 stim_digital_single     = signalD;
%                 stim_digital_periode    = signalD;
%                 clearvars signalD
...
%                             mkdir(strcat(Path.output,filesep,'results',filesep,'StimulationProperties', filesep, experiment.name, filesep));
%                         end
%                         save(strcat(Path.output, 'results', filesep, 'StimulationProperties', filesep, experiment.name, filesep, 'StimulationProperties_raw2','.mat'),'StimulationProperties_raw');
%                     end
%                 else
%                     disp('StimProperties was not calculated')
%                 end
