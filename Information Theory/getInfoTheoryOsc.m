function getInfoTheoryOsc(time, signal1, signal2, fs, experiment, save_data, CSC)
...
signal1 = reshape(cutnglued.xn, [], cutnglued.NWindows);
signal2 = reshape(cutnglued.yn, [], cutnglued.NWindows);
...
SampEnt1 = DiffEnt1; SampEnt2 = DiffEnt1;
...
for freq_idx = 1 : length(freq_bands) - 1
    ...
    for n_window = 1 : cutnglued.NWindows
        r = 0.2 * std(LFP1(:, n_window));
        SampEnt1(n_window, freq_idx) = SampEn(2, r, LFP1(:, n_window));
        r = 0.2 * std(LFP2(:, n_window));
        SampEnt2(n_window, freq_idx) = SampEn(2, r, LFP2(:, n_window));
...
InfoTheory = struct;
InfoTheory.SampEnt1 = SampEnt1;
InfoTheory.SampEnt2 = SampEnt2;
...
if save_data == 0
    disp('DATA NOT SAVED!');
elseif save_data==1
    if ~exist(strcat(path.output,filesep,'results',filesep,'InfoTheory',filesep,experiment.name));
        mkdir(strcat(path.output,filesep,'results',filesep,'InfoTheory',filesep,experiment.name));
    end
    save(strcat(path.output,filesep,'results',filesep,'InfoTheory',filesep,experiment.name,filesep,num2str(CSC)),'InfoTheory')
end
end
